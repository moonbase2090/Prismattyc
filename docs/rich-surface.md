# Rich surface protocol 0.3

This reference defines the wire format, limits, ownership, and input rules.
Rich rendering remains optional and experimental. Query capabilities before
sending rich content. See [capability discovery](capability-protocol.md) and
[client integration](rich-client.md) for the implemented features.
...
guest size, and sends SIGWINCH. The application emits one bounded degradation record and
...
|---|---|---|
| Tasks, runs, diagnostics, action bindings | Application process generation | None in host |
| Shared tree and collection content revisions | Application client | Latest validated bounded snapshot in host/mux |
...
projection. When The application must compute a filter or window, the reply is transient,
...
The application repeats those checks before mutation. Reserved host chords are never
