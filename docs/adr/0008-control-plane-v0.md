# ADR-0008 — Local mux control plane v0

**Status:** Accepted (amended for session verbs, output-activity coalesce,
stale-socket probe, ShutdownServer, window/tab verbs, RenameWindow,
MailAttentionSet/Clear, ReportFocus, per-pane input ledger,
SubscribePane)
**Date:** 2026-08-11
**Depends on:** ADR-0007
...
The mux needs an automation surface that does not scrape terminal pixels. It
must expose stable mux IDs and geometry, survive slow observers, and tell a
client when its incremental view can no longer be trusted. The transport is
local-only. Controller leases and detach remain separate decisions.
...
Per [ADR-0011](0011-long-lived-mux-server.md) this is termination, not
...
[ADR-0009](0009-controller-leases.md).

### Mail attention and gated inject

Mail attention is a **mux-client** verb (`client_id` registered; no pane
lease). “Operator-class” in this ADR means that mux `client_id`, not a
human operator.
...
per-pane log frames on the same connection (PT-77). The handler runs on
...
The server is local same-user automation. [ADR-0011](0011-long-lived-mux-server.md)
...
- CLI, Termwright control tests, and other adapters can consume stable JSON
  without learning Prismattyc's Rust internals.
- Polling bounded event batches is deterministic and naturally supports
  reconnect/resync. `SubscribePane` is the additive v1 push path for
  pane-log content; topology events stay on `Events { after_sequence }`.
- Focus remains client-local. `focus_suggested` is advisory and never becomes a
  server-side focused-pane field. Session switch and window (tab) switch are
  the same: client-local view, not a destroy of the other session or window.

## Rejected alternatives

- Shell command strings: ambiguous quoting and an unsafe automation boundary.
- Screen scraping: loses stable IDs, ordering, and structured failures.
- Unbounded subscriber queues: a slow reader can exhaust memory or stall work.
- Global focus in `Domain`: violates multi-client semantics.
